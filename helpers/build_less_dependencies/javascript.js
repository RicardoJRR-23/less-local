...
const kvs_handler_code = `const { KeyValueStorageDB } = require('lessSqliteDB');
const topics = #topics-import#;

const client = new KeyValueStorageDB();
...
const { KeyValueStorageDB } = require('lessSqliteDB');
  ...
  async () => {
    try {
      ...
      if (items.length) {
        console.log(
          '#less-local-info-tag#',
          \`The items (\${items.map(item => item.id).join(', ')}) were deleted from kvs after reaching their ttl limit.\`
        );
      }
    } catch(error) {
      console.error('System::CRON:Error When Trying to delete items from kvs: ', error);
    }
  },
  null, 
  false,
...
const build = (config) => {
  ...
  fs.writeFileSync(
    ...
    kvs_handler_code.replace('#topics-import#', topics_import)
  );
